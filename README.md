...
编辑 `index.html`，按 HTML 注释提示修改简介、署名、捐款用途列表，并将收款码图片放到项目根目录，命名为 `wechat-qr` / `alipay-qr`（后缀支持 `.webp`、`.png`、`.jpg`、`.jpeg`，按此顺序自动探测，放哪种格式都行，文件名前缀保持一致即可自动显示）。爱发电等链接在 `link-list` 区块修改或取消注释启用。
